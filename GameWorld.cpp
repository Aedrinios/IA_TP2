...
//#endif
	...
	Vehicle* pLeader = new AgentLeader(this,
		SpawnPos,                 //initial position
		RandFloat()*TwoPi,        //start rotation
		Vector2D(0, 0),           //velocity
		Prm.VehicleMass,          //mass
	...
	for (int a = 1; a < Prm.NumAgents; ++a)
	{
		...
		Vehicle* pVehicle = new AgentPoursuiveur(this,
			m_Vehicles[a - 1]->Pos() + offset,  //initial position
			RandFloat()*TwoPi,        //start rotation
			Vector2D(0, 0),           //velocity
			Prm.VehicleMass,          //mass
			Prm.MaxSteeringForce,     //max force
...
void GameWorld::HandleKeyPresses(WPARAM wParam)
{
	...
	case 'Z':
		if (m_Vehicles[0]->Steering()->isArriveOn()) {
			SetCrosshair(Vector2D(m_Vehicles[0]->Pos().x, m_Vehicles[0]->Pos().y - 200));
		}
		break;

	case 'Q':
		if (m_Vehicles[0]->Steering()->isArriveOn()) {
			SetCrosshair(Vector2D(m_Vehicles[0]->Pos().x - 200, m_Vehicles[0]->Pos().y));
		}
		break;
